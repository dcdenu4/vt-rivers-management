...
# IMPORTANT: If using same directory for multiple runs, all files are 
# overwritten
#workspace_dir = os.path.join(
#    "C:", os.sep, "Users", "ddenu", "workspace", "feature_build")
workspace_dir = os.path.join(
    "C:", os.sep, "Users", "ddenu", "Workspace", "UVM", "feature_build")
...
rga_data_path = os.path.join(tabular_data_path, 'Missisquoi', 
                         'Missisquoi_P1P2_All_Miss_Projects.xls')
# Define Digital Elevation Map (DEM) Raster Path
dem_path = os.path.join(data_dir_path, 'Geospatial', 'ElevationDEM_VTHYDRODEM',
                        'vthydrodem', 'hdr.adf')

# Where the project workbooks are located for the watershed of interest
workbook_parent_dir = os.path.join(tabular_data_path, 'Missisquoi', 
                                   'Workbooks')
...
watershed_path = os.path.join(data_dir_path, 'Geospatial', 
    'Missisquoi', 'Missisquoi_Watershed.shp')

# Path for Assessed Phase 2 shapefile
segment_line_path = os.path.join(data_dir_path, 'Geospatial', 
                                 'SGA_Phase_2_Assessed_Reaches', 
                                 'SGA_Phase_2_Assessed_Reaches.shp')
# Path for Vermont hydrography shapefile
hydrography_stream_path = os.path.join(data_dir_path, 'Geospatial', 
...
# Get pebble counts
utils.get_pebble_counts(workbook_parent_dir, pebble_count_path, 
                        'Segment Pebble Counts', rga_data_path)
...
def cm_to_meters(in_array):
    return np.where(in_array!=dem_nodata, in_array * 0.01, dem_nodata)
...
pygeoprocessing.raster_calculator(
        [(dem_path, 1)], cm_to_meters, dem_meters_path,
        gdal.GDT_Float32, dem_nodata)
...
project_projection = watershed_info['projection']

# Output path for projected phase 2 assessed shapefile
segment_line_projected_path = os.path.join(workspace_dir, 'Phase2_Reaches_Projcted.shp')
...
reach_slopes_path = os.path.join(workspace_dir, 'Phase_2_Assessed_Reaches_Slopes.shp')
...
rga_df = pd.read_excel(rga_data_path)
...
elev_diff_df = pd.read_csv(elev_diff_csv_path, index_col=False)
...
slope_df = segment_length_df.merge(elev_sub_diff_df, how='left', 
    left_on='SGAT_PID_P2', right_on='SGAT_ID')
...
geoutils.calculate_drainage_area(
        ...
        hydrography_stream_projected_path, darea_csv_path,
        pour_points_path)
...
utils.calculate_vc_vcratio(upstream_csv_path, 'SGAT_PID_P2', 'upstream_ID', vc_csv_path)
...
utils.calculate_streampower(ssp_path, darea_csv_path, slopes_path, rga_data_path)
utils.calculate_streampower_balance(ssp_balance_path, ssp_path, upstream_csv_path)
...
segments_df = pd.DataFrame(columns=new_features)
...
segments_df['pArmor'] = (rga_df['bank_revetment_length_left'] + rga_df['bank_revetment_length_right']) / (2 * rga_df['segment_length'])
# If no Phase 2 revetment, then use Phase 1 armoring left / right values
phase1_armor_na = (rga_df['BankArmoringLengthLeft'] + rga_df['BankArmoringLengthRight']) / (2 * rga_df['segment_length'])
...
pebble_df = pd.read_csv(pebble_count_path)
...
ssp_df = pd.read_csv(ssp_path)
ssp_bal_df = pd.read_csv(ssp_balance_path)
...
da_df = pd.read_csv(darea_csv_path)
...
som_features_list = ('SGAT_PID_P2', 'DA', 'd84_d16', 'd50', 'slope', 'ssp', 
    'ssp_bal', 'VC', 'VC_RATIO', 'pArmor', 'IR', 'ER', 'WtoD', 'nBars', 'nFCs')
